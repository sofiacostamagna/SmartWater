...
const MapaClientes: React.FC = () => {
  ...
  const handleStatusFilterClick = (statusKey: ClientStatus) => {
    ...
    // Alternar el estado del filtro seleccionado
    if (currentFilters.status?.includes(statusKey)) {
      currentFilters.status = currentFilters.status.filter((s) => s !== statusKey);
    } else {
      currentFilters.status = [...(currentFilters.status || []), statusKey];
    }
